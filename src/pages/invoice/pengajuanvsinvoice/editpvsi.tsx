...
import { useParams, useNavigate } from 'react-router-dom'; // Import useParams to get the ID    
...
const EditPVSI = () => {
    ...
    useEffect(() => {
        dispatch(setPageTitle('Edit Pengajuan VS Invoice'));

        // Fetch the existing data for the record    
        const fetchData = async () => {
            setIsLoading(true);
            try {
                const response = await axios.get(`https://audtrax.sinarjernihsuksesindo.id/backend/api/pvsi/${id}`);
                const data = response.data;

                // Debugging: Check the API response    
                console.log('Fetched data:', data);

                // Convert gaji_tanggal to GMT+7    
                const gajiTanggal = new Date(data.gaji_tanggal);
                const gajiTanggalGMT7 = new Date(gajiTanggal.getTime() + (7 * 60 * 60 * 1000)); // Add 7 hours    

                // Convert periode_gaji to YYYY-MM format    
                const periodeGaji = new Date(data.periode_gaji);
                const periodeGajiString = `${periodeGaji.getFullYear()}-${String(periodeGaji.getMonth() + 1).padStart(2, '0')}`; // Format to YYYY-MM    

                // Ensure the date fields are in the correct format    
                setFormData({
                    ...data,
                    gaji_tanggal: gajiTanggalGMT7.toISOString().split('T')[0], // Format to YYYY-MM-DD    
                    periode_gaji: periodeGajiString, // Format to YYYY-MM    
                });

                // Debugging: Check the formData after setting    
                console.log('Form data after setting:', {
                    ...data,
                    gaji_tanggal: gajiTanggalGMT7.toISOString().split('T')[0],
                    periode_gaji: periodeGajiString,
                });
            } catch (error) {
    ...
    const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
        ...
        // Mengambil tahun dan bulan dari periode_gaji    
        const [year, month] = formData.periode_gaji.split('-');

        // Mengatur tanggal ke hari pertama bulan yang ditentukan    
        const formattedPeriodeGaji = new Date(Date.UTC(parseInt(year), parseInt(month) - 1, 1)); // Bulan dimulai dari 0    
        const formattedPeriodeGajiString = formattedPeriodeGaji.toISOString().split('T')[0]; // Format ke YYYY-MM-DD    

        // Mengatur gaji_tanggal untuk GMT+7    
        const gajiTanggal = new Date(formData.gaji_tanggal);
        const gajiTanggalGMT7 = new Date(gajiTanggal.getTime() + (7 * 60 * 60 * 1000)); // Tambahkan 7 jam    

        const updatedFormData = {
            ...formData,
            periode_gaji: formattedPeriodeGajiString, // Update periode_gaji    
            gaji_tanggal: gajiTanggalGMT7.toISOString().split('T')[0], // Format ke YYYY-MM-DD    
        };

        console.log('Submitting form data:', updatedFormData); // Debugging line to check form data    
        try {
            const response = await axios.put(`https://audtrax.sinarjernihsuksesindo.id/backend/api/pvsi/${id}`, updatedFormData);
    ...
    return (
        <div>
            <h1 className='mb-5'><strong>Report Pengajuan vs Invoice Management  |</strong>|    Edit Report</h1>
            <div className="panel">
                <div className="mb-5">
