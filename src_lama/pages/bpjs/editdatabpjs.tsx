...
const EditDataBpjs = () => {      
    ...
    useEffect(() => {      
        dispatch(setPageTitle('Edit Data BPJS'));      
        fetchData(); // Ambil data saat komponen dimuat  
    }, [dispatch]);      
  
    const fetchData = async () => {  
        try {  
            const response = await fetch(`https://audtrax.sinarjernihsuksesindo.id/backend/api/bpjs/${id}`); // Ambil data berdasarkan id  
            const data = await response.json();  
            if (response.ok) {  
                setFormData(data); // Isi formData dengan data yang diambil  
            } else {  
    ...
    const handleSubmit = async (e: React.FormEvent) => {          
        ...
        try {          
            const response = await fetch(`https://audtrax.sinarjernihsuksesindo.id/backend/api/bpjs/${id}`, { // Gunakan PUT untuk update  
                method: 'PUT',          
                headers: {          
            ...
            if (response.ok) {          
                ...
                showToast('success', 'Data BPJS berhasil diperbarui!');          
    ...
    return (      
        <div>      
            <div className="flex items-center justify-between flex-wrap gap-4 mb-5">      
                <h1 className="flex items-center gap-2">EDIT DATA BPJS |</h1>      
