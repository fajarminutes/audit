...
const EditLates = () => {
    ...
    useEffect(() => {
        dispatch(setPageTitle('Edit Lates'));
        fetchLateRecord(); // Fetch the late record when the component mounts  
    }, [dispatch, id]);

    const fetchLateRecord = async () => {
        try {
            const response = await axios.get(`https://audtrax.sinarjernihsuksesindo.id/backend/api/lates/${id}`);
            const data = response.data;

            // Ensure date fields are in the correct format (YYYY-MM-DD)  
            setFormData({
                ...data,
                tglBayar: data.tglBayar ? data.tglBayar.split('T')[0] : '', // Format date for input  
                tglTarget: data.tglTarget ? data.tglTarget.split('T')[0] : '',
                tglKirim: data.tglKirim ? data.tglKirim.split('T')[0] : '',
            });
        } catch (error) {
            console.error('Error fetching late record:', error);
            toast.error('Failed to fetch late record');
        }
    };
    ...
    const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
        e.preventDefault();
        try {
            await axios.put(`https://audtrax.sinarjernihsuksesindo.id/backend/api/lates/${id}`, formData);
            toast.success('Late record updated successfully');
            navigate('/invoice/semualates'); // Redirect to the list of late records      
        } catch (error) {
            console.error('Error updating late record:', error);
            toast.error('Failed to update late record');
        }
    };

    return (
        <div>
            <h1 className="flex items-center mb-5 gap-2"><strong>Lates Management |</strong>Edit Late</h1>
            <div className="panel">
